...
class AdvertisementViewSet(ModelViewSet):
    ...
    def favorite(self, request, pk=None):
        if not bool(request.user and request.user.is_authenticated):
            return HttpResponse('Неавторизованные пользователи не могут управлять избранным')
        method = self.request.method
        user = self.request.user
        favorite = self.get_object()
        if method == 'POST':
            favorites = Favorites.objects.filter(user=user, favorites=favorite).exists()
            if favorites:
                return HttpResponse('Объявление уже было добавлено в избранное')
            elif user == favorite.creator:
                return HttpResponse('Вы не можете добавить свое объявление к себе в избранное')
            else:
                Favorites.objects.create(user=user, favorites=favorite)
                return Response(status=status.HTTP_201_CREATED)
        if method == 'GET':
            favorites = Favorites.objects.filter(pk=pk)
